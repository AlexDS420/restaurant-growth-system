...
create table if not exists public.ros_organizations (
...
create table if not exists public.ros_organization_members (
  organization_id uuid not null references public.ros_organizations(id) on delete cascade,
...
create table if not exists public.ros_venues (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.ros_organizations(id) on delete cascade,
  name text not null check (length(trim(name)) between 2 and 160),
  slug text not null,
...
create table if not exists public.ros_menu_categories (
  id uuid primary key default gen_random_uuid(),
  venue_id uuid not null references public.ros_venues(id) on delete cascade,
  name text not null,
  description text,
...
create table if not exists public.ros_menu_products (
  id uuid primary key default gen_random_uuid(),
  venue_id uuid not null references public.ros_venues(id) on delete cascade,
  category_id uuid not null references public.ros_menu_categories(id) on delete restrict,
  name text not null,
  description text,
  sku text,
...
create table if not exists public.ros_option_groups (
  id uuid primary key default gen_random_uuid(),
  venue_id uuid not null references public.ros_venues(id) on delete cascade,
  name text not null,
...
create table if not exists public.ros_options (
  id uuid primary key default gen_random_uuid(),
  option_group_id uuid not null references public.ros_option_groups(id) on delete cascade,
  venue_id uuid not null references public.ros_venues(id) on delete cascade,
  name text not null,
...
create table if not exists public.ros_product_option_groups (
  product_id uuid not null references public.ros_menu_products(id) on delete cascade,
  option_group_id uuid not null references public.ros_option_groups(id) on delete cascade,
  primary key (product_id, option_group_id)
);

create table if not exists public.ros_customers (
  id uuid primary key default gen_random_uuid(),
  venue_id uuid not null references public.ros_venues(id) on delete cascade,
  name text not null,
...
create table if not exists public.ros_orders (
  id uuid primary key default gen_random_uuid(),
  venue_id uuid not null references public.ros_venues(id) on delete restrict,
  customer_id uuid references public.ros_customers(id) on delete set null,
...
create table if not exists public.ros_order_items (
  id uuid primary key default gen_random_uuid(),
  order_id uuid not null references public.ros_orders(id) on delete cascade,
  venue_id uuid not null references public.ros_venues(id) on delete restrict,
  product_id uuid references public.ros_menu_products(id) on delete set null,
  name_snapshot text not null,
  unit_price_minor bigint not null check (unit_price_minor >= 0),
  quantity integer not null check (quantity > 0),
  line_total_minor bigint not null check (line_total_minor >= 0),
  ros_options_snapshot jsonb not null default '[]'::jsonb,
  notes text
);

create table if not exists public.ros_payments (
  id uuid primary key default gen_random_uuid(),
...
create table if not exists public.ros_payment_events (
  id uuid primary key default gen_random_uuid(),
  payment_id uuid references public.ros_payments(id) on delete set null,
  venue_id uuid not null references public.ros_venues(id) on delete restrict,
...
create table if not exists public.ros_audit_logs (
  id bigint generated by default as identity primary key,
  organization_id uuid references public.ros_organizations(id) on delete set null,
  venue_id uuid references public.ros_venues(id) on delete set null,
...
create index if not exists idx_ros_venues_org on public.ros_venues(organization_id);
create index if not exists idx_categories_venue_order on public.ros_menu_categories(venue_id, sort_order);
create index if not exists idx_products_venue_visible on public.ros_menu_products(venue_id, is_visible, is_available, sort_order);
create index if not exists idx_ros_orders_venue_status on public.ros_orders(venue_id, status, placed_at desc);
create index if not exists idx_ros_order_items_order on public.ros_order_items(order_id);
create index if not exists idx_ros_payments_venue_status on public.ros_payments(venue_id, status, created_at desc);
create index if not exists idx_ros_payment_events_status on public.ros_payment_events(status, received_at);
create index if not exists idx_audit_venue_created on public.ros_audit_logs(venue_id, created_at desc);

-- Helpers son SECURITY INVOKER y no consultan user_metadata editable por el cliente.
create or replace function public.is_org_member(target_org uuid)
returns boolean language sql stable security invoker set search_path = public
as $$ select exists (
  select 1 from public.ros_organization_members m
...
returns boolean language sql stable security invoker set search_path = public
as $$ select exists (
  select 1 from public.ros_venues v
  join public.ros_organization_members m on m.organization_id = v.organization_id
...
do $$ declare t text; begin
  foreach t in array array['ros_organizations','ros_organization_members','ros_venues','ros_menu_categories','ros_menu_products','ros_option_groups','ros_options','ros_product_option_groups','ros_customers','ros_orders','ros_order_items','ros_payments','ros_payment_events','ros_audit_logs'] loop
    execute format('alter table public.%I enable row level security', t);
    execute format('revoke all on table public.%I from anon, authenticated', t);
  end loop;
end $$;

grant select on public.ros_venues, public.ros_menu_categories, public.ros_menu_products, public.ros_option_groups, public.ros_options, public.ros_product_option_groups to anon;
grant select, insert, update, delete on table
  public.ros_organizations, public.ros_organization_members, public.ros_venues,
  public.ros_menu_categories, public.ros_menu_products, public.ros_option_groups,
  public.ros_options, public.ros_product_option_groups, public.ros_customers,
  public.ros_orders, public.ros_order_items, public.ros_payments,
...
comment on table public.ros_payment_events is 'Idempotencia y trazabilidad de webhooks; validar firma en el BFF antes de insertar.';
comment on table public.ros_audit_logs is 'Registro append-only lógico; las escrituras deben ocurrir desde el BFF con service_role server-side o RPC controlada.';
