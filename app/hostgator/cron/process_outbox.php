...
try {
    ...
    $db->update('ros_outbox_events', ['status'=>'eq.processing', 'locked_at'=>'lt.'.gmdate('c', time() - 600)], ['status'=>'failed', 'next_attempt_at'=>$now, 'last_error'=>'Lock recuperado por cron', 'updated_at'=>$now]);
    $events = $db->query('ros_outbox_events', ['status'=>'in.(pending,failed)', 'next_attempt_at'=>'lte.'.$now, 'order'=>'created_at.asc', 'limit'=>(string)MAX_BATCH, 'select'=>'*']);
    $sent = 0; $failed = 0;
    foreach ($events as $candidate) {
        // El filtro status=pending hace que solo el primer cron concurrente pueda reclamarlo.
        $claimed = $db->update('ros_outbox_events', ['id'=>'eq.'.rawurlencode((string)$candidate['id']), 'status'=>'in.(pending,failed)'], ['status'=>'processing', 'attempts'=>(int)$candidate['attempts'] + 1, 'locked_at'=>$now, 'updated_at'=>$now]);
        if (!$claimed) continue;
        $event = $claimed[0] ?? $candidate;
        try {
            send_outbox_webhook($event);
            $db->update('ros_outbox_events', ['id'=>'eq.'.rawurlencode((string)$event['id']), 'status'=>'eq.processing'], ['status'=>'sent', 'sent_at'=>gmdate('c'), 'last_error'=>null, 'updated_at'=>gmdate('c')]);
            $sent++;
            ...
            $db->update('ros_outbox_events', ['id'=>'eq.'.rawurlencode((string)$event['id']), 'status'=>'eq.processing'], ['status'=>$terminal ? 'dead_letter' : 'failed', 'next_attempt_at'=>$terminal ? gmdate('c') : $next, 'last_error'=>substr($error->getMessage(), 0, 500), 'updated_at'=>gmdate('c')]);
