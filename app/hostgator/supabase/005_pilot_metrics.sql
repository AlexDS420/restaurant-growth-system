...
create index if not exists pilot_metric_events_venue_time_idx on public.pilot_metric_events (venue_id, occurred_at desc);
create index if not exists pilot_metric_events_name_time_idx on public.pilot_metric_events (venue_id, metric_name, occurred_at desc);
alter table public.pilot_metric_events enable row level security;
drop policy if exists "pilot_metrics_no_client_access" on public.pilot_metric_events;
create policy "pilot_metrics_no_client_access" on public.pilot_metric_events for all to anon, authenticated using (false) with check (false);
...
revoke all on public.pilot_metric_events from anon, authenticated;
