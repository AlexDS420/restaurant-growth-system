...
usleep(350000);
function call_api(int $port, string $method, string $path, ?array $body = null, string $cookie = ''): array
{
    $ch = curl_init('http://127.0.0.1:'.$port.$path); if ($ch === false) throw new RuntimeException('curl init');
    $headers = ['Content-Type: application/json']; if ($cookie !== '') $headers[] = 'Cookie: '.$cookie;
    curl_setopt_array($ch, [CURLOPT_CUSTOMREQUEST=>$method, CURLOPT_RETURNTRANSFER=>true, CURLOPT_HEADER=>true, CURLOPT_HTTPHEADER=>$headers, CURLOPT_POSTFIELDS=>$body === null ? null : json_encode($body, JSON_THROW_ON_ERROR)]);
    $raw = (string)curl_exec($ch); $status = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE); $headerSize = (int)curl_getinfo($ch, CURLINFO_HEADER_SIZE); $header = substr($raw, 0, $headerSize); $payload = substr($raw, $headerSize); $setCookie = ''; if (preg_match('/^Set-Cookie:\s*(ros_access_token=[^;\r\n]+)/mi', $header, $match)) $setCookie = $match[1]; return [$status, json_decode($payload, true) ?: [], $setCookie];
}
...
try {
    ...
    [$status, $login, $cookie] = call_api($apiPort, 'POST', '/api/v1/auth/customer-login', ['email'=>'cliente@test.pe','password'=>'test-password']); check_true($status === 200 && $cookie !== '', 'login cliente emite sesión HttpOnly');
    [$status, $created] = call_api($apiPort, 'POST', '/api/v1/public/venues/casa-lima/orders', ['items'=>[['product_id'=>'22222222-2222-4222-8222-222222222222','quantity'=>1,'unit_price_minor'=>1]],'idempotency_key'=>'test-key','customer'=>['name'=>'Cliente','phone'=>'999999999'],'fulfillment'=>['type'=>'pickup']], $cookie);
    ...
    [$status, $payment] = call_api($apiPort, 'POST', '/api/v1/public/venues/casa-lima/orders/public-test-token/pay', ['payment_method'=>'yape','operation_code'=>'OP-123'], $cookie); check_true($status === 201 && ($payment['data']['payment_status'] ?? '') === 'pending_verification', 'Yape queda pendiente de verificación');
