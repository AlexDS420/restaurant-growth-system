...
function Admin() {
  ...
  return <main className="admin-shell"><header className="admin-header"><div><p className="eyebrow">{me.name || me.email}</p><h1>Pedidos de hoy</h1><p>{me.role} · operaciones del local · {mode === 'direct' ? 'conexión directa' : 'BFF'}</p></div><button onClick={async () => { if (mode === 'bff') await api('/auth/logout', { method: 'POST', headers: { 'X-CSRF-Token': csrf } }); else localStorage.removeItem(tokenKey); setMe(null); }}>Cerrar sesión</button></header>{error && <p className="field-error" role="alert">{error}</p>}<section className="admin-card" aria-live="polite">{busy && <p>Cargando pedidos…</p>}{!busy && orders.length === 0 && <div className="empty-state"><h2>Sin pedidos pendientes</h2><p>Los pedidos nuevos aparecerán aquí cuando lleguen.</p></div>}{orders.map((order) => <article className="order-row" key={order.id}><div><p className="eyebrow">{order.status} · {order.payment_status}</p><h2>{order.customer?.name || order.customer_name || 'Cliente'} <span className="order-total">{money(order.totals?.total_minor || order.total_minor)}</span></h2><p>{order.items?.map((item) => `${item.qty} × ${item.name}`).join(' · ') || 'Detalle no disponible'}</p></div><div className="order-actions">{order.payment?.status === 'verifying' && <><button onClick={() => confirm(order.payment.id, 'confirmed')}>Confirmar pago</button><button className="quiet" onClick={() => confirm(order.payment.id, 'rejected')}>Rechazar</button></>}</div></article>)}</section></main>;
}
