...
const moduleLabels = { menu: 'Menú y carta', payments: 'Pagos y conciliación', customers: 'Clientes', reservations: 'Reservas', analytics: 'Analítica', inventory: 'Inventario', promotions: 'Promociones', settings: 'Configuración' };
const moduleEndpoints = { menu: ['/menu/categories', '/menu/products'], payments: ['/payments/reconciliation'], customers: ['/customers'], reservations: ['/reservations?date=upcoming'], analytics: ['/analytics/summary'], inventory: ['/inventory'], promotions: ['/promotions'], settings: ['/venue'] };
const moduleData = (section, data) => section === 'menu' ? { categories: data[0] || [], products: data[1] || [] } : data[0] || {};

function ModuleView({ section, busy, setBusy, setError, loadOrders }) {
  const [data, setData] = useState(null);
  useEffect(() => {
    if (!section || section === 'dashboard' || !moduleEndpoints[section]) return undefined;
    let active = true;
    setBusy(true);
...
function Admin() {
  ...
  return <div className="restaurant-dashboard"><aside className="restaurant-sidebar" aria-label="Navegación del restaurante"><a className="restaurant-brand" href="/"><span className="restaurant-brand-mark" aria-hidden="true">✦</span><span>Foodi<span>Pro</span></span></a><p className="sidebar-label">Operación</p><nav>{nav('dashboard', '▦', 'Dashboard')}{nav('orders', '▤', 'Pedidos')}{nav('menu', '♧', 'Menú')}{nav('payments', '◌', 'Pagos')}{nav('customers', '◎', 'Clientes')}{nav('reservations', '◫', 'Reservas')}{nav('analytics', '◒', 'Analítica')}</nav><div className="sidebar-bottom"><button className="sidebar-link" type="button" onClick={() => { setSection('settings'); setError(''); }}><span aria-hidden="true">⚙</span>Configuración</button><button className="sidebar-logout" onClick={logout}><span aria-hidden="true">↪</span>Cerrar sesión</button><div className="sidebar-user"><span className="avatar" aria-hidden="true">{(me.name || me.email || 'R').slice(0, 1).toUpperCase()}</span><span><strong>{me.name || 'Restaurante'}</strong><small>Administrador</small></span></div></div></aside><div className="restaurant-main"><header className="restaurant-topbar"><label className="dashboard-search"><span aria-hidden="true">⌕</span><input type="search" placeholder="Buscar pedidos..." aria-label="Buscar pedidos" /></label><div className="topbar-actions"><button className="icon-button" type="button" aria-label="Cambiar tema">☼</button><button className="icon-button" type="button" aria-label="Notificaciones">♧</button><span className="topbar-account">{me.name || me.email}</span></div></header>
    ...
    </main><ModuleView section={section} busy={busy} setBusy={setBusy} setError={setError} loadOrders={() => load({ mode, csrf })} /><footer className="admin-footer"><span>Turno operativo · {me.role}</span><span>Restaurant OS · {mode === 'direct' ? 'conexión directa' : 'BFF'}</span></footer></div></div>;
}
