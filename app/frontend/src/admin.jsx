...
function Admin() {
  ...
  const load = async (session = { mode, csrf }) => { setBusy(true); setError(''); try { if (session.mode === 'direct') { const loaded = await directLoad(); setMe(loaded.profile); setOrders(loaded.orders); setMode('direct'); } else { const profile = await api('/me'); if (profile.user?.role === 'customer') throw new Error('Esta cuenta de cliente solo puede acceder a la carta.'); setMe(profile.user); setCsrf(session.csrf); setMode('bff'); const [rows, reconciliation] = await Promise.all([api('/orders?date=today'), api('/payments/reconciliation')]); const payments = Object.fromEntries((reconciliation.payments || []).map((payment) => [payment.order_id, payment])); setOrders(rows.map((order) => ({ ...order, payment: payments[order.id] || null }))); } } catch (e) { setMe(null); if (!/Inicia sesión/i.test(e.message)) setError(e.message); } finally { setBusy(false); } };
