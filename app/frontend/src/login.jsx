...
const destinationFor = (role, requested) => { if (role === 'restaurant') return './admin.html'; if (requested === '/cliente.html' || requested?.startsWith('/carta.html')) return requested; return './carta.html?venue=casa-aurora'; };
function Login() {
    ...
    try {
      ...
      if (role === 'restaurant') {
        ...
        if (bff.ok && bffBody.data?.user) {
          location.href = destinationFor('restaurant', params.get('return')); return;
        }
      ...
      if (role === 'customer') {
        ...
        if (bff.ok && bffBody.data?.user) {
          ...
          location.href = destinationFor('customer', params.get('return')); return;
        }
      ...
      // El acceso de cliente siempre debe pasar por el BFF para validar role:
      // el fallback directo de Supabase no puede garantizar que el JWT sea de
      // un comensal y podría abrir el dashboard equivocado.
      if (!directEnabled || role === 'customer') throw new Error('La configuración de acceso aún no está disponible.');
      ...
      location.href = destinationFor(role, params.get('return'));
