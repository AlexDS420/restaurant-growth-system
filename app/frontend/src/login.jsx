    ...
    try {
      ...
      if (role === 'restaurant') {
        ...
        if (bff.ok && bffBody.data?.user) {
          ...
          const destination = safeReturn && !safeReturn.startsWith('/restaurante.html') ? safeReturn : './admin.html';
          location.href = destination; return;
        }
      ...
      const requestedReturn = params.get('return'); const safeReturn = requestedReturn && requestedReturn.startsWith('/') && !requestedReturn.startsWith('//') ? requestedReturn : null;
      const destination = role === 'restaurant' && (!safeReturn || safeReturn.startsWith('/restaurante.html')) ? './admin.html' : (safeReturn || './cliente.html');
